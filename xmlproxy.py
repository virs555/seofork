...
class XmlParser:
    ...
    def __init__(self):
        ...
        self.XML_PROXY = 'https://xmlproxy.ru/search/xml'

    def __get_xml(self, query, domain, region):
        params = self.params
        params['query'] = query
        params['lr'] = region
    ...
    def get_position(self, query, domain, region='213'):
        query = str(query)
        xml_doc = self.__get_xml(query, domain, region)
        if xml_doc:
        ...
        return False
classP = XmlParser()
print(classP.get_position('купить молоко', 'lenta.com'))
